...
> 虚继承也用到 virtual 关键字，但虚函数和虚继承没有任何关系。

## 2. 重写的定义

子类重新定义父类中对应的虚函数，就叫做子类重写/覆盖了父类的虚函数。
...
多态就是不同对象执行同一行为产生不同结果。具有继承关系的对象才会产生多态，父子对象或者多个子对象，通过父类指针引用，调用重写的成员函数。

### 3.1 多态的条件

**子类必须对父类中的虚函数进行重写，通过父类的指针或引用调用重写函数**。必须是父类的指针引用，只有父类的指针引用才能被子类对象赋值。
...
构成多态的话，才回去考虑重写虚函数。如果不构成多态，不需要考虑重写虚函数的概念。
...
p->test() 把子类对象指针传给 test 函数 this 形参，但 test 函数 this 类型是父类指针，相当于父类指针指向子类对象。

父子类 func 函数构成重写，this->func() 构成多态，所以调用子类的 func。

重写不会改变函数声明，所以 i 值为 1。
...
虚函数表本质是函数指针数组，构成多态时，编译器通过虚表查找函数地址。
...
- 对象指针引用调用可以构成多态，因为父类指针引用指向子类对象空间拿到子类虚表，再去查找虚函数的地址。
