...
<img src="类和对象.assets/const类调用出错权限放大示例.png"  style="zoom: 67%;" />
...
- C++11允许在成员声明处给缺省值，如果该变量没有初始化，就会使用该缺省值。
...
Date d = 2022;
--------------
Date tmp(2022); // 创建临时变量并初始化
Date d(tmp);    // 利用临时变量创建并初始化新对象
~~~

编译器会将这一过程，优化成一次构造函数。也就是直接用该值去调用构造函数。

如果不想发生这样的隐式类型转换，可以用`explicit`修饰构造函数：

<img src="类和对象.assets/explicit修饰构造函数避免发生隐式类型转换示例.png" style="zoom:67%;" />
