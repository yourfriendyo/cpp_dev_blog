...
| 线程是进程的执行分支 | PCB指向进程的部分代码和数据，一个进程可以分为多个执行流      |
| 线程是进程的一部分   | 进程指的是线程所有资源的总和，多个线程组成了进程             |
| 线程粒度更细更轻量化 | 线程相当于轻量版的进程，拥有单个PCB和部分的地址空间          |
| 线程调度成本低       | 线程共享地址空间，所以代码数据大概率不变，不用重新加载进cache |
...
| 进程                                   | 线程                                            |
| -------------------------------------- | ----------------------------------------------- |
| 进程是代码数据和进程内核数据结构的总和 | 线程只占有进程的部分资源                        |
| 进程是承担分配系统资源的基本实体       | 线程是CPU调度的基本单位，承担进程资源的基本单位 |
| 进程具有独立性，其资源大多都是独立的   | 线程在进程内部运行，其资源大多都是共享的        |

**信号是发送给进程的，向进程发送号信号会影响所有线程。**

线程的资源大多都是共享的，但也存在私有资源如 pcb、栈结构、上下文数据。

<br>

“以前的”进程是内部只有一个线程的进程，“现在的”进程是内部可以有多个线程的进程。

> 有些操作系统如windows为线程单独设计线程控制块，但Linux是复用进程pcb模拟线程。

### 1.3 线程的优缺点

| 优点                 | 解释                                                         |
| -------------------- | ------------------------------------------------------------ |
| **创建线程的成本低** | 系统不需设计线程控制块及其算法，不需要维护线程和进程的复杂关系 |
| **线程切换更快**     | CPU调度线程只需要切换上下文数据。调度进程需要把进程内核数据结构全部加载 |
| **线程占用的资源少** | 线程只占用部分进程数据                                       |
| IO等待时间重叠       | IO多数时间都在等待，故让线程等待IO的时间重叠可以适当提升效率 |
| **缺点**             | **解释**                                                     |
| **健壮性降低**       | 线程缺乏保护机制，很有可能一个线程出错导致整个进程崩溃       |
| **缺乏访问控制**     | 进程是访问控制的基本粒度，线程中调用某些函数可能对整个进程造成影响 |
| **编写调试难度高**   | 线程之间可能相互影响，多线程程序更加难以定位错误             |

&nbsp;

## 2. 线程控制

Linux提供在地址空间内创建PCB并分配资源的接口。一般使用第三方线程库`pthread`。

> `pthread`库是系统工程师在系统接口之上设计出的原生线程库。
...
LWP就是轻量级进程，两个mythread线程在内核中的ID是不一样的，证明它们是不同的执行流。系统调度时看的是LWP而不是pid。

之前认为CPU以pid进行区分进程也没错，因为当时进程只有单个执行流，pid就等于LWP。 
...
线程函数的返回类型为`void*`，返回的是线程执行的结果，只存在两种情况：正常退出结果正确，正常退出结果错误。

线程等待要获取线程的运行结果，`retval`是输出型参数，用来获取线程退出的返回值。
...
线程等待无法处理异常情况，线程崩溃就是进程崩溃，处理异常是进程任务。

返回类型是`void*`目的是能够返回任意类型变量，线程等待就可以任意获取，只需要稍加强转即可。
...
调用`pthread_cancel`终止线程的返回值默认为`-1`。如果在新线程中将主线程取消掉，那么进程将变成僵尸进程。

### 2.4 线程分离

线程需要等待，否则会造成错误。如果不需要获取线程的退出信息，只想线程运行结束后自动退出，可将该线程分离。

线程分离本质是设置线程的状态，使得线程不需要等待，运行完毕之后自动释放。
...
回到最初的问题：为什么通过`pthread_self`打印结果和`LWP`编号不一样呢？

**LWP是内核中线程的编号，pthread_self的输出结果是pthread库对线程的编号，它实际上是内存地址。**

线程运行存在一些临时数据，故每个线程都要有自己的私有栈结构。线程库是用户级的第三方库，存在用户级线程控制块。

<img src="09-多线程.assets/用户级线程的存储形式.png" style="zoom:50%;" />

`mmap`区域用来存储线程的相关数据，里面有线程的数据结构，临时数据，私有栈结构等。**每个线程结构体的起始地址就作为线程库对线程的编号**即线程ID。
...
<img src="09-多线程.assets/环形队列设计图示.gif" style="zoom: 33%;" />
