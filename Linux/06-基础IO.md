...
| 函数     | pathname   | flags    | mode     | 返回值                         |
| -------- | ---------- | -------- | -------- | ------------------------------ |
| **open** | 文件的路径 | 打开模式 | 文件权限 | 成功返回文件描述符，失败返回–1 |
...
| 函数      | fd         | buf        | count          | 返回值                 |
| --------- | ---------- | ---------- | -------------- | ---------------------- |
| **write** | 文件描述符 | 写入字符串 | 写入的字符个数 | 返回真正写入的字符个数 |
| **read**  | 文件描述符 | 读取数组   | 读取的字符个数 | 返回真正读取的字符个数 |
...
<img src="06-基础IO.assets/系统管理文件所创建的数据结构图示.png" style="zoom:40%;" />
...
<img src="06-基础IO.assets/打印C语言库文件中的文件结构体的fd变量.png"  style="zoom:40%;" />

如下图所示，标准错误和标准输出都能打印出来，但只有标准输出能重定向到文件中，标准错误不能。

<img src="06-基础IO.assets/标准错误与标准输出重定向结果图示示例.png" style="zoom: 50%;" />
