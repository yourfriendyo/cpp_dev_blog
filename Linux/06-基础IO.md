...
**文件是内容和属性的集合**，文件不仅有内容，还有各种属性如权限、拥有者等。所以，任何文件操作都可以分为对文件内容的操作和对文件属性的操作。

打开文件就是将文件相关的属性信息加载到内存中。内存中不仅存在大量的进程，也可能存在大量的文件，**操作系统必然要像管理进程一样将打开的文件管理起来**。

> 如何描述和管理被打开的文件呢？

文件也有文件结构体，用来表示文件相关的属性和内容。操作系统会将多个文件结构体用某种数据结构管理起来。

#### 文件描述符

实际上都是进程在操作文件，管理文件就要维护进程和进程打开文件的关系。我们通过文件描述符来管理进程打开的文件。
