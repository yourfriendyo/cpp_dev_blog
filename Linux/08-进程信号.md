...
本节介绍Linux系统中信号的概念，以信号的生命周期为路线，讨论信号的产生，信号的保存，信号的处理。
...
系统中也存在信号，信号是系统发送给进程的，进程需要在合适的时候，执行信号对应的动作，因此进程必须明确各种的信号的对应的动作。

#### 信号的产生

当然，进程必须能够分辨信号和对指定信号的处理方式，以便在信号到来时能够及时处理。也就是说，**进程在信号产生前就应该具有识别和处理信号的能力**。

> 实际生活人们收到某种信号时，可能并不会立即处理因为此时还在处理其他事情。比如闹钟响起时赖床。

#### 信号的保存

进程在收到信号时也可能无法立即处理，因为<u>当前进程正在处理优先级更高的任务，只能将信号暂存起来，等到合适的时候再进行处理</u>。也就是说，**进程具有暂时保存信号的能力**。

信号本身也是数据，可被暂存在PCB中。所以说，发送信号的本质就是向进程控制块`task_struct`中写入数据。PCB只有系统能够读写，**系统会向上提供信号的多种发送方式，但本质都是系统向进程发送信号**。

&nbsp;

## 2. 信号的产生

Linux 系统信号如图所示：共有62种信号，前31个是普通信号，后31个是实时信号，我们只学习普通信号。
...
- 处理方式：信号具有默认动作，系统调用`signal`能够设置进程对某个信号的处理方式。

```cpp
#include <signal.h>
typedef void (*sighandler_t)(int); // handler 函数指针
sighandler_t signal(int signum, sighandler_t handler);
// 注册进程对信号 signum 的处理动作为函数 handler。函数声明必须与规定一致。
```
...
> 如图所示，输入`^C`对进程发送2号信号`SIGINT`，进程收到就执行信号处理函数。
...
| `^\` | 向进程发送3号信号`SIGQUIT`，默认是终止进程并进行核心转储 Core Dump。 |
...
进程对9号信号的注册无法生效，因为9号信号被系统占有，无法被捕捉。
...
运行带有野指针等内存错误的程序，进程会异常崩溃。实际上是进程收到了11号终止信号`SIGSEVG`。

执行存在段错误程序时CPU出现异常，然后操作系统发现CPU出现异常，故向该进程发出了11号信号。

软件上面的错误，通常会体现在硬件或其他软件上。运行存在异常问题的程序，导致硬软件出现异常，致使操作系统向目标进程发送对应信号。
...
进程退出时，会设置其退出码和退出信号，表明进程退出的结果和原因。必要的话，**系统会进行 core dump，也就是将进程在内存的中的数据以文件的形式转储在磁盘上，以便后期调试。并设置 core dump 标志位**。
...
`waitpid`可以获取子进程的退出信息，退出码，退出信号，还有就是 Core Dump 标志位。如果进程异常退出，且core dump标志位被设置的话，可能会形成core dump文件。

<img src="08-进程信号.assets/waitpid输出型参数status内容示例图示.png" style="zoom: 33%;" />
