...
<font color=green>18. 什么是管道？匿名管道和命名管道有什么区别？</font>

管道是通过读写文件的缓冲区实现的进程间通信的方式。使用的文件是特殊的内存级文件，数据只停留在内核缓冲区不会刷新到磁盘。

匿名管道是通过让父子进程打开同一个文件的方式，让父子进程看到同一份资源。

命名管道是让双方进程通过相同的文件名打开同一个文件，让二者看到同一份资源。

<font color=green>19. 匿名管道的具体原理？</font>

1. 让一个进程分别以读和写方式打开一个文件。
2. 然后让创建子进程，子进程自然继承得该文件的读写文件描述符。
3. 让一方关闭读端另一方关闭写端，以保证管道的单向通信。

<font color=green>20. 管道的特点？</font>

- 管道是一个只能单向通信的信道。
- 管道虽是系统资源，但生命周期随进程。
- 匿名管道只能用于父子通信，命名管道可以用于任意进程的通信。
- 管道自带同步互斥机制，管道里没数据，读端会阻塞，管道写满的话，写端会阻塞。
- 当写入量小于 PIPE_BUF 4KB 时，管道保证写入原子性。

<font color=green>21. 什么是共享内存？共享内存的特点？</font>

共享内存是让参与通信的进程关联上同一块内存，读写该空间以进行通信。共享内存本质就是一块裸露的空间，读写方式就是一堆指针操作。

- 共享内存是由系统管理的系统资源，生命周期随内核，与进程无关。
